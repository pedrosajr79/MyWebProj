...
export const metadata = { title: "Planos — Carrosseiro" };
...
const plans = [
  ...
  },
  {
    ...
    features: [
      "30 carrosséis/mês",
      "15 slides por carrossel",
      "Export PNG + PDF sem marca d'água",
      "Perfis de marca",
  ...
  },
  {
    ...
    features: [
      ...
      "20 slides por carrossel",
      "Temas customizados ilimitados",
      "Histórico ilimitado",
    ...
    <div className="max-w-4xl mx-auto">
        ...
        <div>
          ...
          <p className="text-white/40 text-sm">
            ...
            {profile?.subscription_status && profile.subscription_status !== "inactive" && (
              <span className="ml-2 text-xs text-white/30">({profile.subscription_status})</span>
            )}
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className={`rounded-2xl border p-6 flex flex-col ${plan.highlight ? "border-indigo-500/40 bg-indigo-500/5" : "border-white/10 bg-white/[0.02]"} ${currentPlan === plan.id ? "ring-2 ring-indigo-500" : ""}`}
          >
            {currentPlan === plan.id && (
              <span className="inline-block mb-3 text-xs font-semibold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded-full w-fit">
                Plano atual
              </span>
            )}
